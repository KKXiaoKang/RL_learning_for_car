...
def synced_timestamps_factory(hf_dataset_factory):
    def _create_synced_timestamps(fps: int = 30) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        timestamps = torch.stack([t.detach().clone() if isinstance(t, torch.Tensor) else torch.tensor(t) for t in hf_dataset["timestamp"]]).numpy()
        episode_indices = torch.stack([e.detach().clone() if isinstance(e, torch.Tensor) else torch.tensor(e) for e in hf_dataset["episode_index"]]).numpy()
